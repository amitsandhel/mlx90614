...
#mylog2 = logging.getLogger('MyLogger2')
...
#mylog2.addHandler(handler)

#mylog2.info("*******INFO NEW TEST**************")
...
class MLX90614_IR_sensor():
	'''This class will read the IR temperatue values from the sensor This is the simulator class that mimicks the real simulator
	we use a number generator for this
	'''
	def __init__(self, address):
		...
		#RAW inital ambient and object temperatures
	...
	def read(self):
		...
		the ambient temperature is between 23-24 C and 26-27 C
		the object temperature is between 30-32 C and 34-35C
		use random.choice() to extract random values from a list
		'''
		#generating object value from random number generator
		self.counter+=1
		#print self.counter
		
		#generating the inital tobject and tambient values 
		self.init_tobj_value = random.randrange(15150, 15250) 
		self.init_tamb_value = random.randrange(14800, 14850)
		
		#if inner counter is between 5-10 then use the second number generator
		#then restart the cycle
		if self.counter >=5 and self.counter <=10:
			self.init_tobj_value = random.randrange(15350, 15400) 
			self.init_tamb_value = random.randrange(14970, 15000)
		if self.counter >=11 and self.counter <=15:
			self.init_tobj_value = random.randrange(15000, 15100) 
			self.init_tamb_value = random.randrange(14750, 14900)
			#restarting the cycle if self.counter =10
			if self.counter == 15:
				self.counter=0
				
		mylog2.debug('init tobject value: ' + repr(self.init_tobj_value) + '--' + 'init tamb value: ' + repr(self.init_tamb_value) )
		
		#sleep for 200 ms the time for the sensor
		time.sleep(0.2)
