...
try:
    import mlx.mlx90614
    #from mlx.mlx90614 import Main as myMain2
...
class Main():
    def __init__(self, parser):
        ...
        #sim file 
        self.simfile = sim.mlx90614_sim.Main()  #myMain()
        #store and instantiate memory address
        self.gui_file = wxmain.MyApp
        #real file
        self.myfile = MYMAIN2
        #OPENING CONFIG FILE
        self.configfile = config.config.Main()
        self.configfile.run()
    ...
    def run(self):
        ...
        try:
            ...
            mylog.debug("Main.py Main().run() function successful" )
            mylog.debug("Main.py Main().run() default_command COMMAND value: "  + repr(default_command) )
            
            self.run_sub() #default_command)
            
            #self.mylogging.debug("Main.py Main().run() function successful" )
